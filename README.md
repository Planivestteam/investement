...
Artificial (Google Gemini — nível gratuito, sem cartão de crédito)**, em português europeu: dashboard
de mercado, página de ativo, notícias com classificação de sentimento, sistema de pontuação (0–100),
"Oportunidades de Hoje" e análise de IA com recomendação, probabilidades e explicação justificada.
...
│   │   └── services/  # yahoo.py, scoring.py, gemini.py, cache.py
│   ├── requirements.txt
│   ├── Dockerfile
│   └── .env.example
...
# edita backend/.env e coloca a tua GEMINI_API_KEY (gratuita, ver secção abaixo)
...
## 🔑 Chave gratuita da Google Gemini API

As funcionalidades de IA (análise de ativos, notícias com sentimento, resumo diário, chat) usam a
**Gemini API**, que tem um nível gratuito permanente (sem cartão de crédito, ao contrário da
Anthropic, que só dá créditos de teste). Como obter:

1. Vai a **https://aistudio.google.com/apikey**.
2. Inicia sessão com uma conta Google.
3. Clica em **"Create API key"** (não pede cartão de crédito).
4. Copia a chave (começa por `AIza...`).

Depois coloca-a em `backend/.env`:
...
O nível gratuito tem limites de utilização (pedidos por minuto/dia), suficientes para uso pessoal.
Se ultrapassares o limite, a API devolve erro 429 e a app mostra "análise de IA indisponível de
momento" em vez de inventar conteúdo — por desenho, a IA nunca deve simular dados que não tem.

Sem qualquer chave configurada, a app continua a funcionar normalmente para tudo o que vem do
Yahoo Finance (preços, gráficos, fundamentais, notícias em bruto) — só as secções de IA ficam
indisponíveis.
...
⚠️ O ficheiro `.gitignore` já exclui `.env` — nunca faças commit da tua chave da Gemini API.
...
- **Notícias em português**: o backend já traduz/analisa via Gemini; podes reforçar priorizando
